...
- *Calibrated (design-level — deferred):* `color-contrast` (**236 nodes**: hushed/muted meta, tinted labels/pills/eyebrows) + `link-in-text-block` (10: links by color alone) are accepted in the spec's `A11Y_ALLOW` WITH reasons — a real fix shifts the palette / link styling system-wide and wants the VR goldens as a net (not yet bootstrapped). Everything else gates forward.
...
- *Follow-up (separate from the Phase 2 gates):* burn down the calibrated a11y debt — a `color-contrast` + `link-in-text-block` remediation pass, best done after the VR goldens land so it has the visual net.
