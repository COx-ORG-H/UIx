#!/usr/bin/env node
/**
 * hx-diff — consumer-side drift tool for vendored hx components.
 *
 * Runs in a CONSUMER repo (paths resolve against cwd, not this script):
 *
 *   node hx-diff.mjs record [--dir components/hx] [--lock .hx-lock.json]
 *       sha256-hash every file under --dir into the lockfile:
 *       { version, dir, files: { "<file>": { addedAt: <mtime ISO>, hash } } }
 *
 *   node hx-diff.mjs check [--dir ...] [--lock ...] [--registry <path to built dist/r>]
 ...
 *         forked              hash differs from lock — REQUIRES a `// hx-fork: <reason>`
...
const dirArg = flagValue('--dir', 'components/hx');
const lockArg = flagValue('--lock', '.hx-lock.json');
...
function listFiles() {
  if (!existsSync(dirAbs)) {
    console.error(`hx-diff: directory not found: ${posix(dirArg)}`);
    process.exit(1);
  }
...
function record() {
  ...
  console.log(`hx-diff record: ${files.length} file(s) from ${posix(dirArg)} -> ${posix(lockArg)}`);
}
...
function loadRegistry(registryDir) {
  ...
  if (!existsSync(abs)) {
    console.error(`hx-diff: registry dir not found: ${posix(registryDir)}`);
    process.exit(1);
  }
  for (const f of readdirSync(abs)) {
    ...
    try {
      ...
      console.error(`hx-diff: WARNING skipping unparseable registry JSON: ${f}`);
      continue;
    }
...
const FORK_MARKER = /\/\/ hx-fork: .+/;

function check() {
  if (!existsSync(lockAbs)) {
    console.error(`hx-diff: no lockfile at ${posix(lockArg)} — run \`hx-diff record\` first.`);
    process.exit(1);
  }
  ...
  console.log(`hx-diff check: ${posix(dirArg)} (lock: ${posix(lockArg)}${registryArg ? `, registry: ${posix(registryArg)}` : ''})`);
  for (const [name, list] of Object.entries(buckets)) {
    console.log(`  ${name.padEnd(18)} ${list.length}`);
    for (const rel of list) console.log(`    ${name === 'forked' ? '~' : name === 'clean-but-outdated' ? '<' : '='} ${rel}`);
  }
  if (untracked.length) {
    console.log(`  untracked          ${untracked.length}  (on disk but not in the lock — re-run \`hx-diff record\` to track)`);
  ...
  if (forkViolations.length) {
    console.error('');
    console.error(`hx-diff: ${forkViolations.length} file(s) diverged from the recorded hash WITHOUT a fork marker:`);
    for (const rel of forkViolations) console.error(`  ✗ ${rel}`);
    console.error('');
    console.error('  A vendored hx file was edited locally with no `// hx-fork: <reason>` line in its');
    ...
    console.error('    - re-run `hx-diff record` if this was an intentional registry update, or');
    console.error('    - add `// hx-fork: <why this consumer diverges>` near the top to own the fork.');
    process.exit(1);
  }
}
...
if (cmd === 'record') record();
  ...
  console.error('usage: hx-diff <record|check> [--dir components/hx] [--lock .hx-lock.json] [--registry <dist/r>]');
  process.exit(1);
}
